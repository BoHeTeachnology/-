$(function(){
    ...
    if(user_id){
        ...
        AjaxObj.UsergetOne(function(result){
            if(result.Data.code ==1){
                var account_Phone =result.Data.data.account == '' ? '' : '&nbsp;&nbsp;('+result.Data.data.account+')';
                var list_name = result.Data.data.name ==''? result.Data.data.real_name+'&nbsp;&nbsp;' : result.Data.data.name+'&nbsp;&nbsp;';
                $('#Valuser').html(list_name+result.Data.data.phone+account_Phone);
                $('#Valuser').attr('data-val',result.Data.data.id)
                getEarliestTooth(result.Data.data.id,v1);
            }
        },user_id)
    ...
    $('#types input,#tooth_type input,#tooth_square input,#tag_name input').change(function(){
    ...
    var visit_time = $('#J-xl').val(),
        ...
        if( !ifFun(visit_time,patient_id,doctor_id,clinic_id,record_content,file_data,treatment,tooth_pic,tooth_type,tooth_square,tooth_suggestion,toothpic_id) ){
            ...
            if(label_id ==''){
                ...
                AjaxObj.Recordadd(function(result){
                    if( result.Data.code == 1 ){
                        sessionStorage.setItem('idSee',result.Data.record_id)
                        callback();
                    }else{
                        console.log(result.Data.msg)
                    }
                },visit_time,patient_id,doctor_id,clinic_id,record_content,file_data,label,label_id,treatment,tooth_pic,tooth_type,tooth_square,tooth_suggestion,toothpic_id,type)
            }
        }
}
