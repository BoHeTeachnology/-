...
const LOAD = 'bohe/ask/LOAD';
const LOAD_SUCCESS = 'bohe/ask/LOAD_SUCCESS';
const LOAD_FAIL = 'bohe/ask/LOAD_FAIL';
...
export default function reducer(state = initialState, action = {}) {
    switch (action.type) {
        ...
        case LOAD_SUCCESS:
            return state.merge({ asklist: action.result.data , allpage: action.result.count })
        case LOAD_FAIL:
...
export function LoadedorLoading(state){
    var loaded = false
    var loading = false
    if(state.hasIn(['ask','loaded'])){
        loaded = state.getIn(['ask','loaded'])
    }
    if(state.hasIn(['ask','loading'])){
        loading = state.getIn(['ask','loading'])
    }
