		...
		setTimeout(function() {
	    	...
	    	Ajax.UsergetInfo(function(result2){
				if( result2.Data.code ==1 ){
					Ajax.RecordgetPatient(function(result){
						if(result.Data.code == 1 ){
							if(result.Data.data == '' || result.Data.data == null){
								...
								for(var i=0; i<result.Data.data.length; i++){
									var patient_phone = result.Data.data[i].patient_phone == '' || result.Data.data[i].patient_phone == null ? '' : ' ('+result.Data.data[i].patient_phone+')'
									...
									html+='<p>'+result.Data.data[i].patient_name+patient_phone+'</p>'
...
$(document).keydown(function(e) {
	if (e.keyCode == 13) {
		...
		location.href='http://' + location.host + "/mintwx/html/doctorCaseSearch.html?val="+searchVal;
	}
})
