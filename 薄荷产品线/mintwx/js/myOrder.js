$(function(){
    ...
    $(document.body).infinite().on("infinite", function() {
		...
		setTimeout(function() {
	    	n++;
    		Ajax.AppointmentList(function(result){
				if(result.Data.code ==1 ){
					if( result.Data.data=='' || result.Data.data == null ){
						...
						for(var i=0; i<result.Data.data.length; i++){
							var visit_name = result.Data.data[i].visit_name == '' || result.Data.data[i].visit_name == null ? result.Data.data[i].patient_name : result.Data.data[i].visit_name;
							html+='<div class="weui_cells weui_cells_access">'
						    html+='<a class="weui_cell" href="myOrderInfo.html?id='+result.Data.data[i].id+'">'
						    html+='<div class="weui_cell_hd">'
						    html+='<p>'
			                if(result.Data.data[i].is_return ==0){
			                	if(result.Data.data[i].service_name != '默认'){
			                		html+='<b>'+visit_name+'<em>'+result.Data.data[i].service_name+'</em></b>'
			                	}else{
			                		html+='<b>'+visit_name+'</em></b>'
			                	}
						    	
						    }else{
						    	if(result.Data.data[i].service_name != '默认'){
						    		html+='<b>'+visit_name+'<em>'+result.Data.data[i].service_name+' - 复诊</em></b>'
						    	}else{
						    		html+='<b>'+visit_name+'<em>'+'复诊</em></b>'
						    	}
...
function AppointmentList(patient_name,p,p_len){
	Ajax.AppointmentList(function(result){
		if(result.Data.code == 1 ){
			if(result.Data.data == ''){
				$('#noDatabox').show();
