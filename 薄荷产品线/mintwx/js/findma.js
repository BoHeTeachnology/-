$(function(){
	...
	$('#resetPwd').click(function(){
		...
		if( !phoneFun(phone) || !check(code,pwd) ){
			...
			Ajax.resetPwd(function(result){
				if(result.Data.code == 1 ){
					opCityTip('重置成功')
					setTimeout(function(){
						if(type == 'order'){
							location.href='http://'+location.host+"/mintwx/login.html?type=order";
						}else if(type == 'info'){
							location.href='http://'+location.host+"/mintwx/login.html?type=info";
						}else{
							location.href='http://'+location.host+"/mintwx/login.html"
						}
					},1000); 
