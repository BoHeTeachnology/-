...
use KurtJensen\MyCalendar\Models\Type as MyCalType;
...
class EventForm extends ComponentBase {
	...
	public $typelist; // form select for types
	public $type; // form select for types
	public $nowtype; // form select for types
	...
	protected function onEventForm() {
		...
		$ty = isset($this->myevent->types->first()->id) ? $this->myevent->types->first()->id : 0;
		...
		$this->typelist = $this->page['typelist'] = MyCalType::selector(
				$ty,
				array('class' => 'form-control custom-select',
						'id' => 'Form-field-myevent-type_id')
				);
		$this->type = $this->page['type'] = MyCalType::get();
		if(isset($this->myevent->types->first()->id)){
			$nowtype=$this->myevent->types;
...
// 		}
		...
		if($string){
			...
			if(count($dates)>1){
				foreach($dates as $da){
					...
					$this->myevent->date = $da; 
					$this->myevent->time = post('time');
					$this->myevent->end_time = post('end_time');
					$this->myevent->spacetime = post('spacetime');
					$this->myevent->categorys = post('category_id');
					$this->myevent->types = post('type_id');
					if ($this->allowpublish) {
						$this->myevent->is_published = 1;
					}
			...
			$this->myevent->categorys = post('category_id');
			//Log::info([post('category_id')]);Log::info(post('type_id'));
			$this->myevent->types = post('type_id');
			if ($this->allowpublish) {
	...
	protected function onDelete() {
		...
		//$this->myevent->delete();
			$this->myevent->update(['is_published' => 0]);
...
//Log::info('ssss'.$end_at);
		...
		$end_at->addMinutes('06')->addHours('03');
