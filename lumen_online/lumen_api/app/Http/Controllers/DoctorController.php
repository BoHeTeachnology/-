	...
	public function doctorlist() {
		...
		if($have==1){
			...
			if($evevt&&count($evevt)>0){
				...
				$docid=implode(',',$docid);
				$result = DB::table ( 'bohe_article_doctor' )->whereRaw ( "user_id in ($docid) " )->paginate ( $num, $columns = [
						'*'
				], $pageName = 'page', $page );
			}else{
				$result = DB::table ( 'bohe_article_doctor' )->where ('id','=',0)->paginate ( $num, $columns = [
		...
		if ($name != null) {
			$result = DB::table ( 'bohe_article_doctor' )->whereRaw ( "name like '%$name%' " )->paginate ( $num, $columns = [ 
					'*' 
			], $pageName = 'page', $page );
		}
		
		if ($name == null &&$cat == null&&$have==null) {
			$result = DB::table ( 'bohe_article_doctor' )->paginate ( $num, $columns = [ 
					'*' 
			], $pageName = 'page', $page );
		}
