$(function(){
    ...
    //查看，判断盒子是否显示
    if( sessionStorage.getItem('infosee_show') =='1' ){
        $('#infobox_see,#edit_but_see').show();
        $('#infobox_add,#save-but').hide();
    }else{
        $('#infobox_add,#save-but').show();
        $('#infobox_see,#edit_but_see').hide();
    }
    //点击编辑按钮，展示出可编辑状态
    $('#edit_but_see').click(function(){
        sessionStorage.removeItem('infosee_show')
        $('#infobox_see,#edit_but_see').hide();
        $('#infobox_add,#save-but').show();
        SeeFun()
    })
...
function addFun(callback){
    var mint_name = $('#mint_name').val(),
        account = $('#account').val(),
        name = $('.yongHuming').val(),
        phone = $('#phone').val(),
        password = $('.Mima').val() ==''?'123456':$('.Mima').val(),
    ...
    */
    if( !ifFun(account,phone,password,card_id,email,society_number) ){
        return false;
    }else{
        if( sessionStorage.getItem('user_id') ){
            ...
            if(mint_name ==''){
                ...
                AjaxObj.Usersave(function(result){
                    if( result.Data.code == 1 ){
                        ...
                        sessionStorage.setItem('infosee_show','1')
                        setTimeout(function(){
                            $('.opacity-tip').hide();
                            callback()
                        },1000)
                        ...
                        var name = $('.yongHuming').val() == '' ? '' : $('.yongHuming').val();
                        $('#user_name').html( name )
                ...
                },sessionStorage.getItem('user_id'),identity_id,mint_name,photo,account,name,phone,password,real_name,sex,birth,race,married,card_type,card_id,have_child,company_name,email,job,society_number,post_address,remark)
            }
        }else{
            AjaxObj.UserAdd(function(result){
                if( result.Data.code == 1 ){
                    ...
                    if(result.Data.msg == '用户已存在！'){
                        ...
                        opCityTip('account',result.Data.msg)
                    }else{
                        opCityTip('',result.Data.msg)
                    }
                }
            },identity_id,photo,account,name,phone,password,real_name,sex,birth,race,married,card_type,card_id,have_child,company_name,email,job,society_number,post_address,remark)
        }
    }
}
function upPicFun(){
...
function ifFun(account,phone,password,card_id,email,society_number){
    ...
    var user_id = sessionStorage.getItem('user_id')
        ...
        AjaxObj.UsergetOne(function(result){
            if(result.Data.code == 1){
                ...
                if(result.Data.data.mint_name!=''){
                    $('#mint_name').attr('disabled',false)
                }
                $('#mint_name').removeClass('readonly')
