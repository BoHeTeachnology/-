...
@connect(
    state => {
        return {
            case_template_error: state.getIn(['case_template','error'])
        }
    }, { pushState: push })
export default class ErrorCenter extends Component {
  ...
  render(){
       ...
       var msg = '';
       var msglength = 0;
       console.log(this.props.case_template_error);
       if(this.props.case_template_error&&this.props.case_template_error.hasIn(['post_success'])){
           msg = this.props.case_template_error.getIn(['msg']);
           this.props.case_template_error.getIn(['post_success'])();
       }else if(this.props.case_template_error&&this.props.case_template_error.hasIn(['pos'])){
           let pos = this.props.case_template_error.getIn(['pos']).toJS();
           let size = this.props.case_template_error.getIn(['pos']).size
           let path_obj;
           let path_name = '';
