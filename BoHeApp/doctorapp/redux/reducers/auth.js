    ...
    if ((typeof window === 'undefined')||(window.__SERVER__ == true)) { ///server side
      ...
      console.log(req);
        if (req.query && req.query.token) ///  微信的鉴权要素是openid
        {
            ...
            // server_token='eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJvZmZpY2lhbC5jb20iLCJhdWQiOiJkb2N0b3IuY29tIiwiaWF0IjoxNTA4NzQ4OTkyLCJuYmYiOjE1MDg3NDg5OTIsImV4cCI6Mjg2NTc0ODk5Miwic3ViIjo2MjkwNzMzNjI3ODI3NDIxMTg0fQ.g1Sf427DtmEXwbePMAcDAmQCuKYTFQBUHn3hB45WLyM'
             console.log(server_token);
            console.log("HHHHHHdsdsHH");

        } else if (req.cookies.khantoken) { /// 浏览器访问的鉴权要素是 cookie token

            console.log('dehuidehui222555');
            server_token = req.cookies.khantoken;
            console.log(server_token)

        } else { // server side none key to login
          console.log('LOAD_FAIL222555');
            return {
                type: LOAD_FAIL,
