...
define('UC_USER_PHONE_FORMAT_ILLEGAL', -7);
define('UC_USER_PHONE_EXISTS', -8);
...
class usercontrol extends base {
    ...
    if(($status = $this->_check_phone($phone)) < 0) {
		...
		$uid = $_ENV['user']->add_user($username, $password, $email, $phone, 0, $questionid, $answer, $regip);
		return $uid;
  }

 function _check_phone($phone, $username = '') {
		if(!$_ENV['user']->check_phoneformat($phone)) {//检测格式是否正确
			return UC_USER_PHONE_FORMAT_ILLEGAL;
		} elseif($_ENV['user']->check_phoneexists($phone, $username)) {//检测是否已经注册过
	...
	function onlogin() {
		...
		return array($status, $user['username'], $password, $user['email'], $user['mobile'], $merge);
	}
