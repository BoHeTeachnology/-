...
class Events extends ComponentBase {
	...
	public function importProperties($comp) {
		...
		$this->doctor_id = $comp->property('doctor_id', 0);
			...
			$occurs = Ocurrs::with(
				array($relation_name => function ($query) {
					...
					if ($this->usePermissions) {
						...
						$query->orwhere("user_id",$this->userId())->orwhere("user_id",$this->doctor_id);
					}
				})
			)->
...
// 				}
		...
		if ($this->property('raw_data', false)) {
			...
			return $this->page['ev'] = [
				'id' => $ocurrs->$relation_name->id,
				'user_id' => $ocurrs->$relation_name->user_id,
				//添加当前用户ID的查询
				'nowuid' => $this->userId(),
...
// 				: $ocurrs->start_at->format($timeFormat) . ' - ' . $ocurrs->end_at->format($timeFormat),
				'time' =>$ocurrs->$relation_name->time,
				'end_time' =>$ocurrs->$relation_name->end_time,
				...
				'tys' => $ocurrs->$relation_name->types->lists('name'),
